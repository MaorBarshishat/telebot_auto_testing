...
CORRECT_PHOTO_HASHED = "10ce70127c9dcd8bdfa017920af67ba9"
...
options.device_name = 'Samsung Galaxy S6'
...
class TestAppium(unittest.TestCase):
    def setUp(self) -> None:
        try:
            self.driver = webdriver.Remote(appium_server_url, options=options)  # connect to the appium server
    ...
    def send_message(self, message):
        ...
        # wait.until(EC.visibility_of_element_located((AppiumBy.XPATH, f'''//*[@text='Message']'''))).send_keys(message)

        # send the message
        element = wait.until(EC.visibility_of_element_located((AppiumBy.ACCESSIBILITY_ID, 'Send')))
        element.click()

    def check_received_message(self, type_message_expected):
        """
        ...
        """
        sleep(10)        # Wait 10 seconds to the message received
